...
import { joinVideos, convertToYouTube, processAudio, extractClip, changeSpeed, imageToVideo, getVideoInfo } from './video-processor';
...
async function runJoin(directoryOrFiles: string | string[], output: string, target?: { width: number; height: number }) {
    const spinner = ora(chalk.blue('Joining videos...')).start();
    try {
        const outputPath = path.resolve(output);
        if (typeof directoryOrFiles === 'string') {
            await joinVideos(path.resolve(directoryOrFiles), outputPath, target);
        } else {
            await joinVideos(directoryOrFiles, outputPath, target);
        }
...
async function promptJoin() {
    ...
    const spinner = ora(chalk.dim('Checking video resolutions...')).start();
    ...
    await runJoin(files, output, target);
}
